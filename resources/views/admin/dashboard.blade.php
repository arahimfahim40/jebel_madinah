                ...
                <div class="col-lg-3 col-md-6 col-xs-12">
                    <div class="box box-block bg-white tile tile-1 mb-2">
                        ...
                        <div class="t-content">
                            ...
                            <i class="fa fa-caret-up text-success mr-0-5"></i><span><a
                                    href="{{route('invoices.index')}}">View All Invoices</a></span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 col-xs-12">
                                ...
                                <tbody>
									@foreach ($vehicleSummary as $index => $vehicle)
										<tr>
											...
											<td>
												<a href="{{route('vehicles.index')}}?status=pending&location_id={{ $vehicle->point_of_loading_id }}">
													{{ $vehicle->pending }}
												</a>
											</td>
											<td>
												<a href="{{route('vehicles.index')}}?status=on_the_way&location_id={{ $vehicle->point_of_loading_id }}">
													{{ $vehicle->on_the_way }}
												</a>
											</td>
											<td>
												<a href="{{route('vehicles.index')}}?status=on_hand_no_title&location_id={{ $vehicle->point_of_loading_id }}">
													{{ $vehicle->on_hand_no_title }}
												</a>
											</td>
											<td>
												<a href="{{route('vehicles.index')}}?status=on_hand_with_title&location_id={{ $vehicle->point_of_loading_id }}">
													{{ $vehicle->on_hand_with_title }}
												</a>
											</td>
											<td>
												<a href="{{route('vehicles.index')}}?status=shipped&location_id={{ $vehicle->point_of_loading_id }}">
													{{ $vehicle->shipped }}
												</a>
											</td>
											<th>
